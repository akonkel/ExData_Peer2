...
It looks like motor vehicle emissions, as indexed by the 'onroad' category, have dropped a fair amount since 1999.
...
balByYear <- balCars %>% group_by(year) %>% summarize(total = sum(Emissions)) %>% mutate(city=rep('Baltimore',dim(balByYear)[1]))
laByYear <- laCars %>% group_by(year) %>% summarize(total = sum(Emissions)) %>% mutate(city=rep('LA',dim(laByYear)[1]))
